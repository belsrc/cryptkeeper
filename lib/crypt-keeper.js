...
CryptKeeper.prototype.base64Encode = function(val) {
  if(!val) {
    throw new Error('val can not be null');
  }
...
CryptKeeper.prototype.randomHex = function(numBytes) {
  if(!numBytes) {
    throw new Error('number of bytes can not be null');
  }
...
CryptKeeper.prototype.pbkdf2Hash = function(val, salt, rounds, keylen, digest, encoding) {
  ...
  if(!salt) {
    throw new Error('salt can not be null');
  }

  if(!rounds) {
    throw new Error('rounds can not be null');
  }
